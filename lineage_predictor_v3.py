...
def process_joint_vcf(vcf_file, lineage_references):
    ...
    with open(vcf_file, 'r') as f:
        for line in f:
            ...
            for i, sample in enumerate(sample_names, start=9):
                ...
                # Define criteria for "presence" of a variant.
                # Here, if the genotype is not homozygous reference ("0/0") or missing ("./.", "."),
                # we consider the variant present.
    ...
    for sample, snps in sample_snps.items():
        ...
        # Determine predicted lineage based on the highest probability.
...
def main():
    parser = argparse.ArgumentParser(
        description="Predict lineage from VCF files using SNP matching probabilities. "
                    "Supports both single-sample VCFs and joint-genotyped VCFs."
    )
    ...
    # Define your lineage references dictionary.
    lineage_references = {
    ...
    all_results = []
    # If processing joint VCFs with multiple samples, call process_joint_vcf.
