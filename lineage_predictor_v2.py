...
def extract_snp_positions(vcf_file, output_txt):
    ...
    Returns the path to the output file or None if extraction fails.
    """
...
def str_conv(file):
    """
    Converts a SNP position file to a set of strings.
    Assumes the file has one SNP position per line.
    """
    ...
    if df.empty:
        ...
        return set()
    # Convert the first column to string and return as a set.
    return set(df[0].astype(str).tolist())

def process_vcf(vcf_file, lineage_references):
    """
    Process a VCF file to compute matching probabilities against lineage references.
    Returns a dictionary with lineage probabilities, the predicted lineage, and the sample name.
    """
    ...
    # Convert SNP file to a set of strings and then to integers
    ...
    # Determine the predicted lineage based on the highest probability
    probabilities = [float(prob.strip('%')) for prob in lineage_probabilities.values()]
    ...
    # Add the predicted lineage and sample name to the results.
...
def main():
    ...
    lineage_references = {
        ...
        'bovis': {4038403, 354054, 652935, 2716806, 45193, 3371401, 3877256, 1647117, 2444952, 2643109, 4034981, 62768, 3022532, 1800521, 1462220, 3623372, 2782927, 437465, 2844761, 648667, 3723227, 4229470, 602207, 1137632, 3199071, 2767842, 3323617, 1344741, 1671658, 1813494, 3667193, 905082, 1492605},
    ...
    # Create DataFrame and save combined results if any are available.
    if all_results:
